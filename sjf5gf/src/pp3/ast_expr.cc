...
void CompoundExpr::BuildChildren(int indentLevel) {
   if (left) left->Build(indentLevel+1);
   op->Build(indentLevel+1);
   right->Build(indentLevel+1);
}
...
void ArrayAccess::BuildChildren(int indentLevel) {
    base->Build(indentLevel+1);
    subscript->Build(indentLevel+1, "(subscript) ");
  }
...
FieldAccess::FieldAccess(Expr *b, Identifier *f) 
 ...
 void FieldAccess::BuildChildren(int indentLevel) {
    if (base) base->Build(indentLevel+1);
    field->Build(indentLevel+1);
  }
...
void NewArrayExpr::BuildChildren(int indentLevel) {
    size->Build(indentLevel+1);
    elemType->Build(indentLevel+1);
}
